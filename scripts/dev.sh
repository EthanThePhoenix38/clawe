#!/bin/bash
set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(dirname "$SCRIPT_DIR")"

cd "$ROOT_DIR"

# Auto-generate .env if it doesn't exist
if [ ! -f .env ]; then
    echo "==> Creating .env with auto-generated OPENCLAW_TOKEN..."
    cp .env.example .env

    TOKEN=$(openssl rand -hex 32)

    # Cross-platform sed (macOS vs Linux)
    if [[ "$OSTYPE" == "darwin"* ]]; then
        sed -i '' "s/change-me-to-secure-random-token/$TOKEN/" .env
    else
        sed -i "s/change-me-to-secure-random-token/$TOKEN/" .env
    fi

    echo "==> Generated OPENCLAW_TOKEN: ${TOKEN:0:8}..."
    echo ""
    echo "NOTE: You still need to set NEXT_PUBLIC_CONVEX_URL in .env"
    echo ""
fi

echo "==> Starting Clawe development environment..."

# Check Docker is running
if ! docker info > /dev/null 2>&1; then
    echo "ERROR: Docker is not running. Please start Docker first."
    exit 1
fi

echo "==> Starting OpenClaw container..."
docker-compose up -d openclaw

echo "==> Waiting for OpenClaw to be ready..."
until curl -s http://localhost:18789/health > /dev/null 2>&1; do
    printf "."
    sleep 1
done
echo ""
echo "==> OpenClaw is ready!"

echo "==> Starting Clawe (Next.js)..."

# Export env vars from .env so Next.js receives the same OPENCLAW_TOKEN
set -a
source .env
set +a

pnpm dev
